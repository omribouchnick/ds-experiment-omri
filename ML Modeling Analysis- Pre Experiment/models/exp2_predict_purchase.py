...
# 1. LOAD DATA
print("\n=== 1. LOAD DATA ===")
raw_df = pd.read_csv('data/exp2_raw.csv')
...
# 12.6. MODEL FAILURE ANALYSIS
print("\n=== 12.6. MODEL FAILURE ANALYSIS ===")
if best_model_name in results:
    best_model_results = results[best_model_name]
    y_pred_best = best_model_results['y_pred']
    
    # Analyze false negatives (missed purchases)
    false_negatives = X_test[y_test != y_pred_best][y_test == 1]
    false_positives = X_test[y_test != y_pred_best][y_test == 0]
    
    print(f"\nModel Failure Analysis:")
    print(f"  False Negatives (missed purchases): {len(false_negatives)}")
    print(f"  False Positives (wrong purchase predictions): {len(false_positives)}")
    
    if len(false_negatives) > 0:
        print(f"\nCharacteristics of Missed Purchases (False Negatives):")
        # Analyze key features for false negatives
        key_features = ['purchase_rate_rolling_7', 'purchase_rate_rolling_21', 'difficulty_mean_rolling_7']
        key_features = [f for f in key_features if f in false_negatives.columns]
        
        for feat in key_features:
            fn_mean = false_negatives[feat].mean()
            overall_mean = X_test[feat].mean()
            print(f"    {feat}: FN avg={fn_mean:.3f}, Overall avg={overall_mean:.3f}")
    
    if len(false_positives) > 0:
        print(f"\nCharacteristics of Wrong Purchase Predictions (False Positives):")
        # Analyze key features for false positives
        for feat in key_features:
            fp_mean = false_positives[feat].mean()
            overall_mean = X_test[feat].mean()
            print(f"    {feat}: FP avg={fp_mean:.3f}, Overall avg={overall_mean:.3f}")

# 12.7. DIFFICULTY-PURCHASE RELATIONSHIP ANALYSIS
print("\n=== 12.7. DIFFICULTY-PURCHASE RELATIONSHIP ANALYSIS ===")
print("Analyzing the relationship between difficulty (stimulus strength) and purchase decisions...")

# Create a dataframe with difficulty and purchase data for analysis
analysis_df = raw_df[['id', 'trial', 'difficulty', 'pd']].copy()
analysis_df = analysis_df.dropna()

# Group by difficulty ranges and calculate purchase rates
difficulty_bins = [0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0]
difficulty_labels = ['0-0.5', '0.5-1.0', '1.0-1.5', '1.5-2.0', '2.0-2.5', '2.5-3.0', '3.0-3.5', '3.5-4.0', '4.0+']

analysis_df['difficulty_bin'] = pd.cut(analysis_df['difficulty'], bins=difficulty_bins, labels=difficulty_labels, include_lowest=True)

# Calculate purchase rates by difficulty level
difficulty_purchase_analysis = analysis_df.groupby('difficulty_bin')['pd'].agg(['count', 'sum', 'mean']).reset_index()
difficulty_purchase_analysis.columns = ['difficulty_range', 'total_trials', 'purchases', 'purchase_rate']
difficulty_purchase_analysis['purchase_rate_pct'] = difficulty_purchase_analysis['purchase_rate'] * 100

print("\nPurchase Rate by Difficulty Level (Stimulus Strength):")
print("=" * 60)
for _, row in difficulty_purchase_analysis.iterrows():
    print(f"  {row['difficulty_range']:8s}: {row['purchase_rate_pct']:5.1f}% ({row['purchases']:3d}/{row['total_trials']:3d})")

# Calculate correlation between difficulty and purchase
correlation = analysis_df['difficulty'].corr(analysis_df['pd'])
print(f"\nCorrelation between difficulty and purchase: {correlation:.3f}")

# Analyze difficulty patterns for users with different purchase behaviors
high_purchase_users = analysis_df.groupby('id')['pd'].mean().sort_values(ascending=False).head(10).index
low_purchase_users = analysis_df.groupby('id')['pd'].mean().sort_values().head(10).index

print(f"\nDifficulty Patterns Analysis:")
print(f"  High purchase users (top 10): {len(high_purchase_users)} users")
print(f"  Low purchase users (bottom 10): {len(low_purchase_users)} users")

# Compare difficulty distributions
high_purchase_difficulty = analysis_df[analysis_df['id'].isin(high_purchase_users)]['difficulty']
low_purchase_difficulty = analysis_df[analysis_df['id'].isin(low_purchase_users)]['difficulty']

print(f"\nDifficulty Statistics by Purchase Behavior:")
print(f"  High purchase users - Difficulty mean: {high_purchase_difficulty.mean():.3f}, std: {high_purchase_difficulty.std():.3f}")
print(f"  Low purchase users - Difficulty mean: {low_purchase_difficulty.mean():.3f}, std: {low_purchase_difficulty.std():.3f}")

# Test if the difference is statistically significant
from scipy import stats
if len(high_purchase_difficulty) > 0 and len(low_purchase_difficulty) > 0:
    t_stat, p_value = stats.ttest_ind(high_purchase_difficulty, low_purchase_difficulty)
    print(f"  T-test p-value: {p_value:.4f}")
    if p_value < 0.05:
        print(f"  ** Statistically significant difference in difficulty patterns **")
    else:
        print(f"  No statistically significant difference in difficulty patterns")
