...
for window in rolling_windows:
    ...
    # merged_df[f'noise_rate_rolling_{window}'] = np.nan

    # Initialize rolling features in merged_df only, and keep only non-redundant features
    for window in rolling_windows:
        # Historical agreement with DS (keep only agreement rate, not disagreement)
        merged_df[f'agreement_rate_rolling_{window}'] = np.nan
        # Historical DS performance (keep only ds_correct_rate)
        merged_df[f'ds_correct_rate_rolling_{window}'] = np.nan
        # Historical user performance (keep only user_correct_rate)
...
####################################################################################################
    ...
    merged_df['ds_confidence'] = np.abs(merged_df['stimulus'] - 0.5)  # How far from neutral (0.5)
...
####################################################################################################
    ...
    simpe_feature_columns = [
        ...
        'stimulus', 'ds_confidence',
        'ds_recommends_signal',  # Keep only signal (noise is 1-signal)
        'is_signal_event', 'trial', 'is_first_trial',
        ...
        # Note: score is correlated with TP/performance, already have avg_score_prev_blocks
    ]
    rolling_feature_columns = [
        ...
        *[f'fp_rate_rolling_{w}' for w in rolling_windows],  # Add missing confusion matrix features
...
#################################### Analysis Before Modeling ##############################################################
    ...
    # 11. SIMPLE MODELING WITH GRID SEARCH
    print("\n=== 11. SIMPLE MODELING WITH GRID SEARCH ===")
    ...
    # Define models and their parameters (removed Random Forest - was worst performer)
    models_config = {
        'XGBoost': {
    ...
    for model_name, config in models_config.items():
        ...
        print(f"📊 Parameters to test: {len(config['params'])} combinations")
        ...
        # Predicting
        print(f"🎯 Predicting with best {model_name} model...")
