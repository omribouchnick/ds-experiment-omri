...
def chrono_cv_splits_on_train(df_train, user_col='id', order_cols=('block','trial'), n_folds=3):
    df = df_train.sort_values(list(order_cols)).copy()
    splits = []
    for k in range(1, n_folds):  # (n_folds-1) splits
        ...
        for _, sub in df.groupby(user_col, sort=False):
            ...
            va = range(bins[k], bins[k+1])
            # Use positional indexing instead of index-based indexing
            tr_mask[sub.index[:bins[k]]] = True
            if len(va) > 0 and bins[k+1] <= len(sub):
                va_mask[sub.index[list(va)]] = True
...
def run_xgb_timeaware(df, features, drop=None, tag='full', test_ratio=0.2, n_folds=3):
    ...
    cv_splits = chrono_cv_splits_on_train(df.iloc[tr_idx], 'id', ('block','trial'), n_folds=n_folds)
    # map relative indices to absolute indices
    mapped = [(tr_idx[a], tr_idx[b]) for (a,b) in cv_splits]
    ...
    grid = GridSearchCV(
        ...
        cv=mapped, scoring='f1', n_jobs=-1, refit=True, verbose=0
    )
    grid.fit(Xtr, ytr)
