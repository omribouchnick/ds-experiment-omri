...
RAW_PATH = "data/exp1_raw.csv"              # <-- change to /mnt/data/... when running here
...
XGB_PARAMS = dict(random_state=SEED, eval_metric="logloss", n_jobs=-1, tree_method="hist",
                  n_estimators=150, max_depth=6, learning_rate=0.1, subsample=1.0, reg_lambda=1.0)

def ece(y, p, n_bins=15):
    bins = np.linspace(0,1,n_bins+1); idx = np.digitize(p, bins)-1; out=0.0
...
def compute_p_signal(df, pi, center=CENTER_M):
    pi = float(np.clip(pi, 1e-6, 1-1e-6))
    z = np.log(pi/(1-pi)) + df['system_d'].astype(float)*(df['stimulus'].astype(float)-center)
    return 1/(1+np.exp(-z))

def load_data():
    raw = pd.read_csv(RAW_PATH); agg = pd.read_csv(AGG_PATH)
...
def prepare_block(raw, agg, block_type="DS"):
    df = raw.query("alert_system==1").copy() if block_type.upper()=="DS" else raw.query("alert_system==0").copy()
    ...
    if dep_col:
        ...
        df = df.merge(a[['id','block','dependency_num_prev_block']].drop_duplicates(), on=['id','block'], how='left')
    else:
        df['dependency_num_prev_block'] = np.nan
    return df

def add_rolling(df, use_posterior=True):
    ...
    if has_ds: cols += ds_bases
    if use_posterior and 'p_signal' in df: cols += ['p_signal_mean']
    for w in ROLLING_WINDOWS:
        for c in cols: df[f'{c}_rolling_{w}'] = np.nan
    ...
    for uid, sub in df.groupby('id', sort=False):
        ...
        for w in ROLLING_WINDOWS:
            s[f'signal_rate_rolling_{w}'] = s['target'].rolling(w,1).mean().shift(1)
            s[f'tp_rate_rolling_{w}'] = s['tp'].rolling(w,1).mean().shift(1)
            s[f'fp_rate_rolling_{w}'] = s['fp'].rolling(w,1).mean().shift(1)
            s[f'fn_rate_rolling_{w}'] = s['fn'].rolling(w,1).mean().shift(1)

            s[f'system_tp_rate_rolling_{w}'] = s['system_tp'].rolling(w,1).mean().shift(1)
            s[f'system_fp_rate_rolling_{w}'] = s['system_fp'].rolling(w,1).mean().shift(1)
            s[f'system_fn_rate_rolling_{w}'] = s['system_fn'].rolling(w,1).mean().shift(1)

            s[f'user_tp_given_system_signal_rate_rolling_{w}'] = s['user_tp_given_system_signal'].rolling(w,1).mean().shift(1)
            s[f'user_tp_given_system_noise_rate_rolling_{w}']  = s['user_tp_given_system_noise'].rolling(w,1).mean().shift(1)

            if has_ds:
                s[f'ds_tp_rate_rolling_{w}'] = s['ds_tp'].rolling(w,1).mean().shift(1)
                s[f'ds_fp_rate_rolling_{w}'] = s['ds_fp'].rolling(w,1).mean().shift(1)
                s[f'ds_fn_rate_rolling_{w}'] = s['ds_fn'].rolling(w,1).mean().shift(1)

            if use_posterior and 'p_signal' in s:
                s[f'p_signal_mean_rolling_{w}'] = s['p_signal'].rolling(w,1).mean().shift(1)
        df.loc[s.index, s.columns] = s
    return df

def build_feature_sets(df, block_type="DS"):
    base = ['p_signal','trial','is_first_trial','block','is_first_block','purchase_ds_block_num','system_d','dependency_num_prev_block']
...
def split_pooled_chrono(df, test_ratio=0.2):
    mask = np.zeros(len(df), dtype=bool)
    for _, sub in df.groupby('id', sort=False):
        n = len(sub); t = max(1, int(np.floor(n*test_ratio)))
        mask[sub.index[-t:]] = True
    return np.flatnonzero(~mask), np.flatnonzero(mask)

def split_pooled_fixed(df, train_trials=30, test_trials=20):
    tr, te = [], []
    for _, sub in df.groupby('id', sort=False):
        n = len(sub); 
        if n < train_trials + test_trials: continue
        tr.extend(sub.iloc[:train_trials].index.tolist())
        te.extend(sub.iloc[train_trials:train_trials+test_trials].index.tolist())
    return np.array(tr), np.array(te)

def split_holdout_ids(df, frac=0.7):
    users = df['id'].drop_duplicates().sample(frac=1.0, random_state=SEED).tolist()
    k = int(len(users)*frac)
...
def fit_eval(Xtr, ytr, Xte, yte):
    if HAVE_XGB:
        ...
        clf = HGB(random_state=SEED, learning_rate=0.1, max_iter=200)
    clf.fit(Xtr, ytr)
...
# (…main that runs all four modes + ablations…)
