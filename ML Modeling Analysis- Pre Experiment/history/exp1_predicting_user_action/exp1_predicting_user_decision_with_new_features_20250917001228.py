# Experiment 1: Predicting user signal/noise decisions with NEW FEATURES (DS BLOCKS)
# This includes the new system and conditional user features for apples-to-apples comparison
...
OUT = Path('exp1_outputs_new_features'); OUT.mkdir(exist_ok=True)
...
# DS confusion matrix
raw_df['ds_tp'] = ((raw_df['ds_recommends_signal']==1) & (raw_df['event_type']=='S')).astype(int)
raw_df['ds_fp'] = ((raw_df['ds_recommends_signal']==1) & (raw_df['event_type']=='N')).astype(int)
raw_df['ds_tn'] = ((raw_df['ds_recommends_signal']==0) & (raw_df['event_type']=='N')).astype(int)
raw_df['ds_fn'] = ((raw_df['ds_recommends_signal']==0) & (raw_df['event_type']=='S')).astype(int)

# Conditional user TP rates (NEW FEATURES)
raw_df['user_tp_given_ds_alarm'] = ((raw_df['user_action']=='S') & (raw_df['event_type']=='S') & (raw_df['ds_recommends_signal']==1)).astype(int)
raw_df['user_tp_given_ds_no_alarm'] = ((raw_df['user_action']=='S') & (raw_df['event_type']=='S') & (raw_df['ds_recommends_signal']==0)).astype(int)
...
# Additional conditional features (DS correctness-based)
raw_df['user_tp_given_ds_correct'] = ((raw_df['user_action']=='S') & (raw_df['event_type']=='S') & (raw_df['ds_was_correct']==1)).astype(int)
raw_df['user_tp_given_ds_incorrect'] = ((raw_df['user_action']=='S') & (raw_df['event_type']=='S') & (raw_df['ds_was_correct']==0)).astype(int)

# Additional conditional features (System correctness-based)
raw_df['user_tp_given_system_correct'] = ((raw_df['user_action']=='S') & (raw_df['event_type']=='S') & 
                                          ((raw_df['system_indicates_signal']==1) & (raw_df['event_type']=='S') | 
                                           (raw_df['system_indicates_signal']==0) & (raw_df['event_type']=='N'))).astype(int)
raw_df['user_tp_given_system_incorrect'] = ((raw_df['user_action']=='S') & (raw_df['event_type']=='S') & 
                                            ((raw_df['system_indicates_signal']==1) & (raw_df['event_type']=='N') | 
                                             (raw_df['system_indicates_signal']==0) & (raw_df['event_type']=='S'))).astype(int)

# Complex conditional features (DS + System combinations)
raw_df['user_tp_given_ds_alarm_system_signal'] = ((raw_df['user_action']=='S') & (raw_df['event_type']=='S') & 
                                                  (raw_df['ds_recommends_signal']==1) & (raw_df['system_indicates_signal']==1)).astype(int)
raw_df['user_tp_given_ds_alarm_system_noise'] = ((raw_df['user_action']=='S') & (raw_df['event_type']=='S') & 
                                                 (raw_df['ds_recommends_signal']==1) & (raw_df['system_indicates_signal']==0)).astype(int)
raw_df['user_tp_given_ds_no_alarm_system_signal'] = ((raw_df['user_action']=='S') & (raw_df['event_type']=='S') & 
                                                     (raw_df['ds_recommends_signal']==0) & (raw_df['system_indicates_signal']==1)).astype(int)
raw_df['user_tp_given_ds_no_alarm_system_noise'] = ((raw_df['user_action']=='S') & (raw_df['event_type']=='S') & 
                                                    (raw_df['ds_recommends_signal']==0) & (raw_df['system_indicates_signal']==0)).astype(int)
...
# -------------- Rolling histories (shifted) - WITH NEW FEATURES --------------
rolling_windows = [1,3,7,14,21,40]
for w in rolling_windows:
    for base in ['signal_rate','user_ds_agreement_rate',
                 'tp_rate','fp_rate','fn_rate',  # tn_rate removed (redundant)
                 'system_tp_rate','system_fp_rate','system_fn_rate',  # system_tn_rate removed (redundant)
                 'ds_tp_rate','ds_fp_rate','ds_fn_rate',  # ds_tn_rate removed (redundant)
                 'user_tp_given_ds_alarm_rate','user_tp_given_ds_no_alarm_rate',
                 'user_tp_given_ds_correct_rate','user_tp_given_ds_incorrect_rate',
                 'user_tp_given_system_signal_rate','user_tp_given_system_noise_rate',
                 'user_tp_given_system_correct_rate','user_tp_given_system_incorrect_rate',
                 'user_tp_given_ds_alarm_system_signal_rate','user_tp_given_ds_alarm_system_noise_rate',
                 'user_tp_given_ds_no_alarm_system_signal_rate','user_tp_given_ds_no_alarm_system_noise_rate',
...
for _, idx in merged_df.groupby('id').indices.items():
    ...
    for w in rolling_windows:
        ...
        # User confusion matrix rolling rates (TN removed - redundant)
        ...
        # tn_rate removed (redundant: tn = 1 - tp - fp - fn)
        
        # System confusion matrix rolling rates (TN removed - redundant)
        ...
        # system_tn_rate removed (redundant)
        
        # DS confusion matrix rolling rates (TN removed - redundant)
        ...
        # ds_tn_rate removed (redundant)
        
        # Conditional user TP rolling rates (NEW FEATURES)
        sub[f'user_tp_given_ds_alarm_rate_rolling_{w}'] = sub['user_tp_given_ds_alarm'].rolling(w, min_periods=1).mean().shift(1)
        sub[f'user_tp_given_ds_no_alarm_rate_rolling_{w}'] = sub['user_tp_given_ds_no_alarm'].rolling(w, min_periods=1).mean().shift(1)
        sub[f'user_tp_given_ds_correct_rate_rolling_{w}'] = sub['user_tp_given_ds_correct'].rolling(w, min_periods=1).mean().shift(1)
        sub[f'user_tp_given_ds_incorrect_rate_rolling_{w}'] = sub['user_tp_given_ds_incorrect'].rolling(w, min_periods=1).mean().shift(1)
        ...
        sub[f'user_tp_given_system_correct_rate_rolling_{w}'] = sub['user_tp_given_system_correct'].rolling(w, min_periods=1).mean().shift(1)
        sub[f'user_tp_given_system_incorrect_rate_rolling_{w}'] = sub['user_tp_given_system_incorrect'].rolling(w, min_periods=1).mean().shift(1)
        
        # Complex conditional rolling rates (DS + System combinations)
        sub[f'user_tp_given_ds_alarm_system_signal_rate_rolling_{w}'] = sub['user_tp_given_ds_alarm_system_signal'].rolling(w, min_periods=1).mean().shift(1)
        sub[f'user_tp_given_ds_alarm_system_noise_rate_rolling_{w}'] = sub['user_tp_given_ds_alarm_system_noise'].rolling(w, min_periods=1).mean().shift(1)
        sub[f'user_tp_given_ds_no_alarm_system_signal_rate_rolling_{w}'] = sub['user_tp_given_ds_no_alarm_system_signal'].rolling(w, min_periods=1).mean().shift(1)
        sub[f'user_tp_given_ds_no_alarm_system_noise_rate_rolling_{w}'] = sub['user_tp_given_ds_no_alarm_system_noise'].rolling(w, min_periods=1).mean().shift(1)
...
# -------------- Feature sets (no leak) - WITH NEW FEATURES --------------
base_simple = [
...
print(f"Total features: {len(FEATURES_NO_LEAK)}")
print(f"Base features: {len(base_simple)}")
...
def train_eval_xgb(df, feature_cols, tag, test_ratio=0.2, n_folds=3, calibrate=True, cal_ratio=0.1, cal_method='isotonic'):
    ...
    if calibrate:
        ...
        grid = GridSearchCV(
            ...
            cv=cv_splits, scoring='f1', n_jobs=-1, refit=True, verbose=1
        )
        print(f"Running XGBoost grid search for {tag}...")
        grid.fit(Xtr_core, ytr_core)
        base_model = grid.best_estimator_
        print(f"Best XGBoost params for {tag}: {grid.best_params_}")
...
# ------------------------ Runs (XGB) - WITH NEW FEATURES ------------------------
results = []

# Define feature sets
feats_full = [c for c in FEATURES_NO_LEAK]
...
# Test without all historical data - only current trial features
feats_no_historical = [f for f in FEATURES_NO_LEAK if not any(x in f for x in ['rolling_', 'mean_rolling_'])]
log_featset('no_historical', feats_no_historical)
results.append(train_eval_xgb(merged_df, feats_no_historical, tag='no_historical',
                              calibrate=CALIBRATE, cal_ratio=CAL_RATIO, cal_method=CAL_METHOD))

# Test without DS features (only rolling historical, no current DS info)
feats_no_ds_current = [f for f in FEATURES_NO_LEAK if not any(x in f for x in ['ds_recommends_signal', 'ds_was_correct', 'ds_tp', 'ds_fp', 'ds_fn', 'ds_tn'])]
log_featset('no_ds_current', feats_no_ds_current)
results.append(train_eval_xgb(merged_df, feats_no_ds_current, tag='no_ds_current',
                              calibrate=CALIBRATE, cal_ratio=CAL_RATIO, cal_method=CAL_METHOD))

# Test without system features (only rolling historical, no current system info)
feats_no_system_current = [f for f in FEATURES_NO_LEAK if not any(x in f for x in ['system_indicates_signal', 'system_tp', 'system_fp', 'system_fn', 'system_tn'])]
log_featset('no_system_current', feats_no_system_current)
results.append(train_eval_xgb(merged_df, feats_no_system_current, tag='no_system_current',
                              calibrate=CALIBRATE, cal_ratio=CAL_RATIO, cal_method=CAL_METHOD))

# Test without event_type dependent features (only rolling historical, no current event_type info)
feats_no_event_type_current = [f for f in FEATURES_NO_LEAK if not any(x in f for x in ['tp', 'fp', 'fn', 'tn', 'correct', 'incorrect'])]
log_featset('no_event_type_current', feats_no_event_type_current)
results.append(train_eval_xgb(merged_df, feats_no_event_type_current, tag='no_event_type_current',
                              calibrate=CALIBRATE, cal_ratio=CAL_RATIO, cal_method=CAL_METHOD))

# Test without complex conditional features (only basic conditionals)
feats_no_complex_conditional = [f for f in FEATURES_NO_LEAK if not any(x in f for x in ['ds_alarm_system', 'ds_no_alarm_system', 'ds_correct', 'ds_incorrect', 'system_correct', 'system_incorrect'])]
log_featset('no_complex_conditional', feats_no_complex_conditional)
results.append(train_eval_xgb(merged_df, feats_no_complex_conditional, tag='no_complex_conditional',
                              calibrate=CALIBRATE, cal_ratio=CAL_RATIO, cal_method=CAL_METHOD))
...
try:
    ...
    grid = GridSearchCV(
        ...
        {'iterations':[200,400], 'depth':[4,6], 'learning_rate':[0.1,0.2], 'l2_leaf_reg':[1,3]},
        cv=cv_splits, scoring='f1', n_jobs=-1, refit=True, verbose=1
    )
    print("Running CatBoost grid search...")
    grid.fit(Xtr_core, ytr_core)
    cat_base = grid.best_estimator_
    ...
    print(f"[catboost_full] Acc@0.5={acc:.4f}  F1@0.5={f1:.4f}  AUC={auc:.4f}  Brier={brier:.4f}  ECE={ece:.4f}")
    if CALIBRATE:
