# Experiment 1: Predicting user signal/noise decisions with temporal validation (NON-DS BLOCKS)
...
def train_eval_xgb(df, feature_cols, tag, test_ratio=0.2, n_folds=3, calibrate=True, cal_ratio=0.1, cal_method='isotonic'):
    ...
    if calibrate:
        ...
        # Grid search commented out - using best parameters from last run
        # grid = GridSearchCV(
        ...
        # Use best parameters from last run for each ablation
        if tag == 'full':
            best_params = {'learning_rate': 0.1, 'max_depth': 6, 'n_estimators': 100, 'subsample': 1.0}
        elif tag == 'no_stimulus':
            best_params = {'learning_rate': 0.1, 'max_depth': 6, 'n_estimators': 200, 'subsample': 0.8}
        elif tag == 'no_dsrec':
            best_params = {'learning_rate': 0.2, 'max_depth': 6, 'n_estimators': 100, 'subsample': 1.0}
        elif tag in ['no_stimulus_no_dsrec', 'no_current_stim_bundle']:
            best_params = {'learning_rate': 0.2, 'max_depth': 6, 'n_estimators': 200, 'subsample': 0.8}
        elif tag == 'no_historical':
            best_params = {'learning_rate': 0.2, 'max_depth': 3, 'n_estimators': 100, 'subsample': 0.8}
        else:
            best_params = {'learning_rate': 0.1, 'max_depth': 6, 'n_estimators': 100, 'subsample': 1.0}  # default
...
try:
    ...
    # Use best parameters from last run
    best_params_cat = {'depth': 6, 'iterations': 200, 'l2_leaf_reg': 3, 'learning_rate': 0.1}
