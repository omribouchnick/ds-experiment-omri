...
for _, idx in merged_df.groupby('id').indices.items():
    ...
    for w in rolling_windows:
        ...
        # Conditional user TP rolling rates (NO DS features)
...
# ------------------------ Runs (XGB) - NO DS FEATURES ------------------------
results = []

# Define feature sets (NOTE: system_confidence REMOVED everywhere, NO DS FEATURES)
feats_full = [c for c in FEATURES_NO_LEAK]  # no system_confidence anywhere, no DS features
...
def build_pdf():
    try:
        ...
        W, H = A4
        c.setFont("Helvetica-Bold", 16); c.drawString(40, H-50, "Experiment 1 — User Decision Prediction (NON-DS BLOCKS)")
        c.setFont("Helvetica", 10)
        c.drawString(40, H-70, f"NON-DS blocks only (alert_system == 0)   N={len(merged_df):,}")
