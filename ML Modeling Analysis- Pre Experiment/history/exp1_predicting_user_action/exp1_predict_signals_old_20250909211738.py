...
# Create target variable and basic features
...
print(f"\nTraining XGBoost with best parameters: {best_params['XGBoost']}")
...
print(f"XGBoost completed - Accuracy: {accuracy_xgb:.4f}, F1: {f1_xgb:.4f}, AUC: {auc_xgb:.4f}")

# Train CatBoost with best parameters (if available)
if catboost_available:
    print(f"\nTraining CatBoost with best parameters: {best_params['CatBoost']}")
    ...
    print(f"CatBoost completed - Accuracy: {accuracy_cat:.4f}, F1: {f1_cat:.4f}, AUC: {auc_cat:.4f}")

# Find best model
best_model_name = max(results.keys(), key=lambda x: results[x]['test_f1'])
best_model = results[best_model_name]['model']

print(f"\nBest model: {best_model_name} (F1: {results[best_model_name]['test_f1']:.4f})")
