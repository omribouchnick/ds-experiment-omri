# Write B1 and B2 generator scripts with ONLY the two requested changes.
    ...
    for ps in ps_list:
        for d_h in d_vals:
            for d_s in d_vals:
                ...
                # 4) Compute DS posterior and binary decision (threshold 0.5) using TRUE Ps
                ...
                # Add 100 human evidence values (+6 display shift)  <<< CHANGE #1
                ...
                # Add 100 DS binary decisions (uses true Ps)       <<< CHANGE #2 is "no change" here
...
if __name__ == "__main__":
    ...
    out_file = Path("conditions_experiment_3ps_11x11_100_B1.csv")
    ...
    for ps in ps_list:
        for d_h in d_vals:
            for d_s in d_vals:
                ...
                # 4) Compute DS posterior and binary decision (threshold 0.5) with Ps=0.5  <<< CHANGE #2
...
if __name__ == "__main__":
    # Generate the conditions CSV file
    out_file = Path("conditions_experiment_3ps_11x11_100_B2.csv")
    out_path, shape = generate_conditions_csv(out_file)
    print("Wrote", out_path, "shape=", shape)
'''
...
if __name__ == "__main__":
    ...
    # Write B1 and B2 generator scripts
    p1 = Path("data/generate_conditions_B1.py")
    p2 = Path("data/generate_conditions_B2.py")
    
    p1.write_text(b1_code)
    p2.write_text(b2_code)
    
    print(f"Created: {p1}")
    print(f"Created: {p2}")
    
    # Run both scripts to generate the CSV files
    print("\n=== Running B1 ===")
    exec(b1_code)
