...
for _, idx in merged_df.groupby('id').indices.items():
    ...
    for w in rolling_windows:
        ...
        # Conditional user TP rolling rates
...
def train_eval_xgb(df, feature_cols, tag, test_ratio=0.2, n_folds=3, calibrate=True, cal_ratio=0.1, cal_method='isotonic'):
    ...
    if calibrate:
        ...
        # Grid search with expanded parameter space for new features
        grid = GridSearchCV(
            xgb.XGBClassifier(random_state=42, eval_metric='logloss', n_jobs=-1, tree_method='hist'),
            {'n_estimators':[100,200,300],'max_depth':[3,4,6,8],'learning_rate':[0.05,0.1,0.2,0.3],'subsample':[0.8,0.9,1.0],'colsample_bytree':[0.8,0.9,1.0]},
...
# Test 30-20 split with new features
print("\n" + "="*60)
print("TESTING 30-20 SPLIT WITH NEW FEATURES")
print("="*60)

def test_30_20_split(df, feature_cols, tag_suffix=""):
    """Test 30-20 split (30 trials train, 20 trials test)"""
    X = df[feature_cols]; y = df[TARGET]
    
    # Create 30-20 split per user
    test_mask = np.zeros(len(df), dtype=bool)
    for _, sub in df.sort_values(['id','block','trial']).groupby('id', sort=False):
        n = len(sub)
        if n >= 50:  # Only users with at least 50 trials
            test_mask[sub.index[-20:]] = True  # Last 20 trials for test
    
    tr_indices = np.flatnonzero(~test_mask)
    te_indices = np.flatnonzero(test_mask)
    
    if len(te_indices) == 0:
        print(f"No users with enough trials for 30-20 split")
        return None
        
    Xtr, ytr = X.iloc[tr_indices], y.iloc[tr_indices]
    Xte, yte = X.iloc[te_indices], y.iloc[te_indices]
    
    print(f"30-20 Split: {len(tr_indices)} train, {len(te_indices)} test")
    
    # Use same hyperparameter tuning as main experiment
    cv_splits = chrono_cv_splits_on_train(df.iloc[tr_indices], 'id', ('block','trial'), n_folds=3)
    ...
    print(f"Running XGBoost 30-20 grid search for {tag_suffix}...")
    grid.fit(Xtr, ytr)
    xgb_model = grid.best_estimator_
    print(f"Best XGBoost 30-20 params: {grid.best_params_}")
    
    # Test predictions
    proba = xgb_model.predict_proba(Xte)[:,1]
    pred = (proba >= 0.5).astype(int)
    
    # Metrics
    acc = accuracy_score(yte, pred)
    f1 = f1_score(yte, pred, zero_division=0)
    prc = precision_score(yte, pred, zero_division=0)
    rec = recall_score(yte, pred)
    auc = roc_auc_score(yte, proba)
    
    print(f"[30-20 {tag_suffix}] Acc={acc:.4f} F1={f1:.4f} Prec={prc:.4f} Rec={rec:.4f} AUC={auc:.4f}")
    
    return {
        'tag': f'30_20_{tag_suffix}',
        'acc': acc, 'f1': f1, 'prec': prc, 'rec': rec, 'auc': auc,
        'model': xgb_model, 'features': feature_cols
    }

# Test 30-20 split on full features
results_30_20 = []
# Define feature sets first
feats_full = [c for c in FEATURES_NO_LEAK]  # no system_confidence anywhere
results_30_20.append(test_30_20_split(merged_df, feats_full, 'full'))
...
try:
    ...
    grid = GridSearchCV(
        ...
        {'iterations':[200,400,600], 'depth':[4,6,8,10], 'learning_rate':[0.05,0.1,0.2,0.3], 'l2_leaf_reg':[1,3,5,10], 'colsample_bylevel':[0.8,0.9,1.0]},
        cv=cv_splits, scoring='f1', n_jobs=-1, refit=True, verbose=1
    )
