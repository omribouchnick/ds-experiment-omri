# Experiment 1 – within-user future prediction (no leakage) + ablations + SHAP + calibration + per-user metrics
...
# Calibration ON (isotonic) with a time-aware per-user holdout from training
CALIBRATE = True
CAL_METHOD = 'isotonic'   # 'isotonic' usually best for tree ensembles
CAL_RATIO  = 0.10         # last 10% per-user from TRAIN used for calibration
...
def train_eval_xgb(df, feature_cols, tag, test_ratio=0.2, n_folds=3, calibrate=True, cal_ratio=0.1, cal_method='isotonic'):
    ...
    if calibrate:
        ...
        calib = CalibratedClassifierCV(base_estimator=base_model, method=cal_method, cv='prefit')
        calib.fit(Xcal, ycal)
        clf = calib
...
try:
    ...
    if CALIBRATE:
        cat_cal = CalibratedClassifierCV(base_estimator=cat_base, method=CAL_METHOD, cv='prefit')
    ...
    print(f"[catboost_full] Acc@0.5={acc:.4f}  F1@0.5={f1:.4f}  AUC={auc:.4f}  Brier={brier:.4f}  ECE={ece:.4f}  Best={grid.best_params_}")
    if CALIBRATE:
