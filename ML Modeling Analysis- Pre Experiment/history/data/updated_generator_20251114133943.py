...
def generate_conditions_csv(
    ...
    rows = []
    uid = 1
    total_rows = len(ps_list) * len(d_vals) * len(d_vals)
    ...
    for ps in ps_list:
        for d_h in d_vals:
            for d_s in d_vals:
                ...
                if uid % 50 == 0 or uid == total_rows:
                    print(f"  Generated {uid}/{total_rows} rows...", end='\r')
...
def quick_qc(df: pd.DataFrame, n_trials=N_TRIALS, out_csv=QC_CSV):
    ...
    print("\nQC — absolute mean errors vs targets:")
    # Check if errors are all zero (moment matching enabled)
    all_zero = all(qc[c].dropna().max() < 1e-10 for c in ["abs_err_H|S","abs_err_H|N","abs_err_S|S","abs_err_S|N"])
    if all_zero:
        print("  NOTE: All errors are 0.000 because MATCH_MEANS=True forces exact means.")
        print("        This is intentional - empirical means are adjusted to exactly match ±d'/2.")
...
if __name__ == "__main__":
    import time
    print(f"Generating {3 * 11 * 11} rows (3 priors × 11 d'H × 11 d'S) with {N_TRIALS} trials each...")
    print(f"Moment matching: MATCH_MEANS={MATCH_MEANS}, MATCH_SIGMA={MATCH_SIGMA}")
    start_time = time.time()
    
    df = generate_conditions_csv(OUT_CSV)
    
    elapsed = time.time() - start_time
    print(f"\nWrote {OUT_CSV}  shape={df.shape}")  # Expect (363, 485) without custom DS; +120 if custom DS on
