...
def load_block_trials(ps: float, human_sensitivity: float, system_sensitivity: float) -> dict:
    # Updated to use new CSV format with 120 trials
    ...
    data_dict = {1: {}, 2: {}, 3: {}}
    # Block 1: 10 practice trials (no DS) - hardcoded
    for i in range(1, 11):
        data_dict[1][i] = {'event': 'noise', 'stimuli': 7.043675233241219, 'ds_stimuli': 0.6470356136858931, 'ds_judgment': 1}
    
    # Block 2: 10 practice trials (with DS) - use first 10 trials from CSV
    # Block 3: 100 main trials - use trials 1-100 from CSV

    for index, row in generated_data.iterrows():
         # Block 3: Use first 100 trials (even though CSV has 120)
         # Block 1 & 2 use hardcoded practice trials
         for event_number in range(1, 101):  # Changed to 100 for Block 3
             if event_number < 10:
                 event_number_str = f'0{event_number}'
             else:
                 event_number_str = f'{event_number}'
             event_type = row[f'event_t{event_number_str}']
             human_stimulus = row[f'h_t{event_number_str}']
             system_stimulus = row[f's_t{event_number_str}']
             system_judgment = int(row[f'ds_dec_t{event_number_str}'])  # Ensure it's an integer (0 or 1)
