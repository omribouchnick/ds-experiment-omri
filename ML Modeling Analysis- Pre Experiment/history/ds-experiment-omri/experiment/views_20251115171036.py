...
def load_block_trials(ps: float, human_sensitivity: float, system_sensitivity: float) -> tuple:
    """
    Load trial data from CSV for a user.
    Only selects from rows where used=0.
    Returns: (data_dict, row_id) where row_id is the CSV row ID for marking as used later.
    """
    # Load CSV and filter for unused rows matching parameters
    ...
    if len(available_rows) == 0:
        raise ValueError(f"No available rows for ps={ps}, dH={human_sensitivity}, dS={system_sensitivity}")
    
    # Randomly select one row
    selected_row = available_rows.sample(n=1).iloc[0]
    row_id = int(selected_row['id'])
    
    # Load all 120 trials at once
    data_dict = {1: {}, 2: {}, 3: {}}
    ...
    for trial_num in range(1, 11):
        ...
        data_dict[1][trial_num] = {
            ...
            'stimuli': float(selected_row[f'h_t{t_str}']),
            'ds_stimuli': float(selected_row[f's_t{t_str}']),
            'ds_judgment': int(selected_row[f'ds_dec_t{t_str}'])
        }
    
    # Block 2: Trials 1-10 again (with DS shown)
    for trial_num in range(1, 11):
        t_str = format_trial_num(trial_num)
...
def landing_page(request):
    ...
    events_data, csv_row_id = load_block_trials(request.session["ps"],
...
def toast_2(request):
    ...
    if request.method == 'POST':
        ...
        if 'csv_row_id' in request.session:
            ...
            print(f"DEBUG: Marked CSV row {request.session['csv_row_id']} as used=1")
