#!/usr/bin/env python3
"""
Generate B1 and B2 versions of the conditions CSV with specific changes:

B1 changes:
1) Human display shift: write h_tXX as (x + 6.0) so users don't see negatives
2) Keep DS decisions using the true Ps (Bayes with prior)

B2 changes:
1) Human display shift: write h_tXX as (x + 6.0) so users don't see negatives  
2) DS decisions IGNORE Ps: decide as if prior=0.5 (less conservative when Signals are rare)
"""
...
def arange_grid(start: float, stop: float, step: float):
    ...
    vals = []
    x = start
    eps = 1e-9  # Small epsilon to handle floating point precision
    while x <= stop + eps:
...
def ds_posterior(x_ds, ps, dprime_ds):
    """Compute Bayesian posterior P(Signal|x) using equal-variance Gaussians"""
...
def generate_conditions_csv(out_path: Path, version: str,
    ...
    for ps in ps_list:
        for d_h in d_vals:
            for d_s in d_vals:
                ...
                if version == "B1":
                    # B1: Use true Ps for DS decisions
                    p_ds = ds_posterior(x_s, ps, d_s)
                elif version == "B2":
                    # B2: Use Ps=0.5 for DS decisions (ignore true prior)
                    p_ds = ds_posterior(x_s, 0.5, d_s)
                else:
                ...
                # Add 100 human evidence values (+6 display shift for both B1 and B2)
                for i, x in enumerate(x_h, 1):
                    row[f"h_t{i:02d}"] = float(x + 6.0)
                
                # Add 100 system evidence values (raw)
...
def main():
    """Generate both B1 and B2 versions"""
    print("=== Generating B1 and B2 Conditions CSV Files ===")
    
    # Generate B1 version
    print("\n--- B1 Version ---")
    print("Changes: Human display shift (+6), DS uses true Ps")
    b1_file = Path("conditions_experiment_3ps_11x11_100_B1.csv")
    b1_path, b1_shape = generate_conditions_csv(b1_file, "B1")
    print(f"Wrote {b1_path}, shape={b1_shape}")
    
    # Generate B2 version  
    print("\n--- B2 Version ---")
    print("Changes: Human display shift (+6), DS uses Ps=0.5 (ignores true prior)")
    b2_file = Path("conditions_experiment_3ps_11x11_100_B2.csv")
    b2_path, b2_shape = generate_conditions_csv(b2_file, "B2")
    print(f"Wrote {b2_path}, shape={b2_shape}")
    
    print(f"\n=== Both files generated successfully! ===")
    print(f"B1: {b1_path} (DS uses true prior)")
    print(f"B2: {b2_path} (DS uses Ps=0.5)")
